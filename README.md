...
| --- | --- | --- | --- |
| <img src="docs/icons/voxel3d.png" alt="" width="64" height="64"> | [`voxel3d`](mods/voxel3d/) | 0.6.2 | The overworld as a voxel diorama, textured from the cartridge's own tileset and drawn at the window's resolution, with the ground carrying on past the mesh so the maps around this one stand where the connection graph puts them, their woods and their towns standing on them and the world past every map standing its own, and the fight staged on the map it started on, its trainers walking on and off it, filling the window with it, under a steerable over-the-shoulder camera. Near it is turned solid, far it is the cartridge's own drawing stood up. The grass bends in the wind and so do the flowers in it. One switch, in the game's own MODS menu or on this mod's page, turns it on for both, behind a wipe that covers the build. |
...
| <img src="docs/icons/follower.png" alt="" width="64" height="64"> | [`follower`](mods/follower/) | 0.3.1 | One of your Pokemon out of its ball, walking one cell behind you. Choose it directly from that Pokemon's party menu, configure cycling and surfing, and put it away with one press of a key or of a row in the MODS menu. Face it and press A to pet it, and let it find hidden items if you want it to. |
