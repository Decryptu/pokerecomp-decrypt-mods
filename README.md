...
| --- | --- | --- | --- |
| <img src="docs/icons/voxel3d.png" alt="" width="64" height="64"> | [`voxel3d`](mods/voxel3d/) | 0.6.5 | The overworld as a voxel diorama, textured from the cartridge's own tileset and drawn at the window's resolution, with the ground carrying on past the mesh so the maps around this one stand where the connection graph puts them, their woods and their towns standing on them and the world past every map standing its own, and the fight staged on the map it started on, its trainers walking on and off it, filling the window with it, under a steerable over-the-shoulder camera. Near it is turned solid, far it is the cartridge's own drawing stood up. The grass bends in the wind and so do the flowers in it. The sky takes the hour's own colours out of the cartridge's palette rows and water draws a waterline where it meets its bank, and one LOOK row switches the whole of that against the flat one, with a WATER row for a calm, a rough or a glassy sea and a LEVEL rung on the camera that frames the sky at all. One switch, in the game's own MODS menu or on this mod's page, turns the view on for both, behind a wipe that covers the build. |
