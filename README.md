...
| --- | --- | --- | --- |
| <img src="docs/icons/voxel3d.png" alt="" width="64" height="64"> | [`voxel3d`](mods/voxel3d/) | 0.6.0 | The overworld as a voxel diorama, textured from the cartridge's own tileset and drawn at the window's resolution, with the ground carrying on past the mesh so the maps around this one stand where the connection graph puts them and their woods stand on them, and the fight staged on the map it started on, its trainers walking on and off it, filling the window with it, under a steerable over-the-shoulder camera. Near it is turned solid, far it is the cartridge's own drawing stood up. One switch on its page turns it on for both. |
