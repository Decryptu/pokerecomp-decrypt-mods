...
## Turns a painted `houses.json` into the cartridge's generation's
## `shape/gen<n>/houses.gd`, and says what is wrong with it on the way.

const MOD := "user://mods/voxel3d"
const OUT := "%s/shape/gen%d/houses.gd"
...
func _initialize() -> void:
	...
	if args.size() > 2 and args[2] == "--write":
		var path: String = OUT % [MOD, data.generation]
