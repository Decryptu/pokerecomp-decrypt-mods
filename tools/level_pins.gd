...
func _initialize() -> void:
	...
	for record: Dictionary in (painted as Dictionary)["maps"]:
		...
		for cy: int in cells.y:
			for cx: int in cells.x:
				if walkable[cy * cells.x + cx] == 0 or not _painted(rows[cy][cx]):
					continue
				for step: Vector2i in [Vector2i.RIGHT, Vector2i.DOWN]:
					var to := Vector2i(cx + step.x, cy + step.y)
					if to.x >= cells.x or to.y >= cells.y:
						continue
					if walkable[to.y * cells.x + to.x] == 0 \
							or not _painted(rows[to.y][to.x]):
						continue
					var rise: int = absi(
						MARKS.find(rows[cy][cx]) - MARKS.find(rows[to.y][to.x])
					)
...
func _painted(mark: String) -> bool:
...
##
## A digit is a level, base 36 so a cave's storey fits; `#` is a wall, which
## carries no level of its own; `.` is a cell the paint says nothing about.
