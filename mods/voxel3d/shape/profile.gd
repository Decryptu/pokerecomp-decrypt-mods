...
## tree, which is 187171 of the game's 231633 modelled tiles and 3407 of New Bark
## Town's 3640.
##
## WHAT A PLANT STANDS NEXT TO, counted over every map wearing the tileset. Three
## cheaper rules were built and each was refused by its own picture: the nearest
## flat ground anywhere spread the town square into the wood and paved New Bark's
## forest, the map's commonest perimeter floor is water on any map with a
## shoreline and flooded it, and the nearest ground BY COLOUR is unstable, giving
## the same tree four different floors on four maps of one tileset. Counting
## adjacency per MAP fails for the same reason the first does: New Bark's ring
...
## every other tileset-keyed table in this file is: the tilesets themselves
## differ between Crystal and Gold and Silver, so seven of these numbers are
## approximate on the other two, which is the imprecision the whole profile
## already carries rather than a new one.
##
## A tileset with no entry keeps the drawing's own art, which is what a tileset
## whose plants always have ground beside them never needs.
